    ...
    <div class="row justify__content__around gap40">  
        ...
        <div>
          <ul class="lk__list row gap10">
            ...
            <li><a class="lk__link lk__link__active" href="">Subscribe</a></li>
        ...
        <?php
            if($rights == "admin"){
                echo '<ul>';
                $query = $db->query('SELECT subscribe.email AS `email`, subscribe.date AS `date` FROM subscribe ORDER BY `date`');
                
                while ($row = $query->fetch(PDO::FETCH_OBJ)) {
                        echo '<li><b>' . $row-> date . '<br>' . $row-> email .'</b><br></li>';
                        // echo '<a href="https://account.mail.ru/" target="_blank"><button>Subscribe</button></a>';
                        echo '<a href="mailto:'.$row-> email.'" target="_blank"><button>Subscribe</button></a>';
                }
                  echo '</ul>';
            } ?>
                </div>  
